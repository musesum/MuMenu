...
public class BranchVm: Identifiable, ObservableObject {
    ...
    static func titleForNodes(_ nodes: [FloNode]) -> String {
        let nameFirst = nodes.first?.title ?? ""
        let nameLast  = nodes.last?.title ?? ""
        return nameFirst + "…" + nameLast
    }

    public init(nodes: [FloNode] = [],
                treeVm: TreeVm,
        ...
        self.panelVm = PanelVm(nodes: nodes,
                                 treeVm: treeVm)
    ...
    private func buildNodeVms(from nodes: [FloNode],
                              prevNodeVm: NodeVm?) {

        for node in nodes {

            let nodeVm = NodeVm.cached(node, self, prevNodeVm)
    ...
    func addNodeVm(_ nodeVm: NodeVm?) {
